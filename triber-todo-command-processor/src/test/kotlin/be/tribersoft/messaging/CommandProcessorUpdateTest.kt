...
@ActiveProfiles("test")
class CommandProcessorUpdateTest() {
    @Autowired
    @Qualifier("command")
    ...
    fun createTodoListCommandIsTransformedToCreateTodoListEventThroughActor() {
        val command = UpdateTodoListCommand("name", UUID.randomUUID())
        ...
        await().atMost(5, TimeUnit.SECONDS).until(java.lang.Runnable {
            ...
            val event = JsonPath.from(message.payload as String).getObject("", TodoListUpdatedEvent::class.java)
